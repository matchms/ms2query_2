...
class Fingerprints:
    ...
    def from_spectrum_set(cls, spectrum_set: AnnotatedSpectrumSet, fingerprint_type, nbits):
        ...
        for inchikey, spectrum_indexes in tqdm(
            spectrum_set.spectrum_indices_per_inchikey.items(), desc="Get most common inchi per inchikey"
        ):
            spectra_matching_inchikey = [spectrum_set.spectra[index] for index in spectrum_indexes]
            most_common_inchi = Counter([spectrum.get("inchi") for spectrum in spectra_matching_inchikey]).most_common(
                1
            )[0][0]
    ...
    def combine_fingerprints(
        cls, fingerprints_1: "Fingerprints", fingerprints_2: "Fingerprints", allow_replacing: bool = False
    ) -> "Fingerprints":
        ...
        if fingerprints_1.nbits != fingerprints_2.nbits:
            raise ValueError(
                f"The nbits for the fingerprints do not match: {fingerprints_1.nbits} != {fingerprints_2.nbits}"
            )
        # handle overlap
        ...
        if not np.array_equal(
            fingerprints_1.get_fingerprints(overlapping_inchikeys),
            fingerprints_2.get_fingerprints(overlapping_inchikeys),
        ):
            if allow_replacing:
        ...
        inchikeys_to_add = [
            inchikey for inchikey in fingerprints_2.inchikeys if inchikey not in fingerprints_1.inchikeys
        ]
    ...
    def subset_fingerprints(self, list_of_inchikeys) -> "Fingerprints":
        return Fingerprints(
            self.get_fingerprints(list_of_inchikeys), list_of_inchikeys, fingerprint_type=self.fingerprint_type
        )
